...
// limit 32 characters for hub/project names
output aiHubName string                   = toLower('${take(sanitizedAppName, 5)}-${resourceAbbreviations.cognitiveServicesHub}-${sanitizedEnvironment}${resourceTokenWithDash}')
output aiHubProjectName string            = toLower('${take(sanitizedAppName, 5)}-${resourceAbbreviations.cognitiveServicesHub}-proj-${sanitizedEnvironment}${resourceTokenWithDash}')
...
output containerAppStreamlitName string   = take(toLower('${sanitizedAppName}-${resourceAbbreviations.appContainerApps}-streamlit-${sanitizedEnvironment}${resourceTokenWithDash}'), 32)
output containerAppAgentName string       = take(toLower('${sanitizedAppName}-${resourceAbbreviations.appContainerApps}-agent-${sanitizedEnvironment}${resourceTokenWithDash}'), 32)
output containerChatName string       = take(toLower('${sanitizedAppName}-${resourceAbbreviations.appContainerApps}-chat-${sanitizedEnvironment}${resourceTokenWithDash}'), 32)
...
output maps string                        = toLower('${sanitizedAppName}-maps-${sanitizedEnvironment}${resourceTokenWithDash}')
