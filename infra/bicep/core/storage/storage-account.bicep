...
output primaryEndpoints object = useExistingStorageAccount
  ? existingStorageAccount.properties.primaryEndpoints
  : storage.properties.primaryEndpoints
output containerNames array = [
  for (name, i) in containers: useExistingStorageAccount
    ? null
    : {
        name: name
        url: useExistingStorageAccount
          ? '${existingStorageAccount.properties.primaryEndpoints.blob}/${name}'
          : '${storage.properties.primaryEndpoints.blob}/${name}'
      }
]
